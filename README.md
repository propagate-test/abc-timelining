...
## Pipeline architecture: ingest → vectorise → resolve trigger

Timelining owns the shared data pipeline (ingest and vectorise) and **triggers** resolve on sibling organising apps. Resolver logic, graph writes beyond Entry, and resolve backlog crons live in those apps.

```
Telegram → /api/story/webhook
  ├─ (configured channel) forward → https://{domain}/webhook
  └─ lpush → timelining::ingest::backlog
       → /api/story/ingest → Neo4j Entry
       → triggerResolve (text entries, resolve channels only)
       → /api/story/voice-vectorise → transcribe → triggerResolve (voice)
       → sibling POST /api/webhook/resolve?entryId=...
            → sibling resolver + resolveStatus in Neo4j
            → sibling backlog cron retries failures
```
...
|-------|-----------------|------------------|
| **Ingest** | Redis backlog, `/api/story/ingest`, Neo4j Entry write, immediate `/webhook` forward for channels in `organising.config.ts` | Interactive `/webhook` (e.g. enrol `/ask`, voice ack) |
| **Vectorise** | Transcribe, chunk, embed crons (`/api/story/voice-vectorise`, `/api/story/page-vectorise`) | — |
| **Resolve trigger** | `triggerResolve` after text ingest or voice transcribe → POST `?entryId=` to configured resolve URL | Schema extraction, domain graph writes (Role, etc.), `resolveStatus` updates, backlog cron (`?backlog=true`) |

Anyone can create a `_bot*` channel and messages are ingested into Neo4j. Channels listed in `organising.config.ts` additionally get webhook forwarding and resolve triggers.

Timelining does **not** run a resolve cron. If a resolve trigger fails, each sibling app's own backlog cron retries (e.g. enrol: `POST /api/webhook/resolve?backlog=true`).
...
| Module | Role |
|--------|------|
| `organising.config.ts` | Channel → domain and resolve route mapping |
| `services/webhook/organisingRoute.ts` | Immediate forward to sibling `/webhook` |
